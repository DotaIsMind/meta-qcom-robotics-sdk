...
# Staging root is keyed by recipe: several images inherit this class and can be
# built in the same workspace, and sstate moves the whole of this directory into
# QIRP_SSTATE_OUT_DIR. A shared path would let two images clobber each other's
# intermediate files. The layout under QIRP_SSTATE_OUT_DIR is unchanged.
QIRP_SSTATE_IN_DIR = "${DEPLOY_DIR}/factory/${PN}"
...
# No cleandirs on QIRP_SSTATE_OUT_DIR: it is shared by every image inheriting
# this class, so wiping it would delete the SDKs generated by the other images.
# sstate tracks its own installed files through the task manifest.
